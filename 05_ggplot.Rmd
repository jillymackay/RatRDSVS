...
```{r, message = FALSE, warning = FALSE,  fig.cap="A ggplot object with no geom layers"}
...
We need to tell R to add a `geom` layer, and we do that by adding a `+`^[You may be interested to know that the `+` symbol is a precursor to the `%>%` symbol. Both `ggplot2` and `tidyverse` were mainly written by [Hadley Wickham](https://twitter.com/hadleywickham) who has often spoken about [why ggplot won't ever use the %>%  operator](https://community.rstudio.com/t/why-cant-ggplot2-use/4372/7)] symbol. In this case, we want to add a `geom_point` layer, so we write the following:



```{r, message = FALSE, warning = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat" }
...
```{r, message = FALSE, warning = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat, x axis changed" }
...
```{r, message = FALSE, warning = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat, x and y axes changed" }
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat, black & white theme" }
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat, classic theme" }
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat, adjusted labels" }
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat, regression line added" }
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_point layer, the price of diamonds by their carat and cut, regression line added" }
...
```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_boxplot, the carat of diamonds by their clarity" }
...
diamonds %>% 
  ggplot(aes(x = clarity, y = carat, colour = clarity)) + 
  geom_boxplot() +
  labs(title = "Claritym cut and carat of >50,000 diamonds",
       subtitle = "Data from ggplot2",
       x = "Clarity (clearness) of diamond",
       y = "Carat (weight) of diamond") + 
  theme_classic() +
...
## Bar charts {#gg_bars}
To create a bar chart, we simply need the `geom_bar` argument. 


```{r, message = FALSE, warning = FALSE, echo = FALSE, fig.cap="A ggplot object with a geom_bar, the carat of diamonds by their clarity" }
...
There are a few notes here. 
  ...
  * Within `geom_bar` we have set the argument `geom_bar(stat = "summary")`. This tells R to calculate the mean carat for each group (Clarity within cut)
  
  * `geom_bar` wants `fill = clarity` instead of `colour = clarity`, as it treats `colour` as the line around the bar. 
