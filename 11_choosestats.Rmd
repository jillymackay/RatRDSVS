...
```{block type = "hey"}
This chapter is potentially controversial. I deliberately haven't discussed statistics in great detail in this book, not least because I'm not entirely confident in this area myself. 

Another reason is that Andy Field, Jeremy Miles, and Zoe Field have already covered this in their brilliant [Discovering Statistics Using R](https://www.discoveringstatistics.com/books/discovering-statistics-using-r/) book. 
```
...
As many, many, [many]() folk say, *correlation is not causation*. In statistical terms, that means that correlation does not aim to predict y from x, only to see if there is an underlying structure to the variation.
